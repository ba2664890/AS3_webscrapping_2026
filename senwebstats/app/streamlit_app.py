...
/* ═══════════════════════════════════════
   DESIGN TOKENS — GLASSMORPHISM GOLD
═══════════════════════════════════════ */
:root {
  --bg:           #EDE5D0;
  --canvas:       linear-gradient(145deg,#E8DEC8 0%,#DDD3B8 35%,#E4DAC4 70%,#EAE0CB 100%);
  --card:         rgba(255,255,255,0.78);
  --glass-light:  rgba(255,255,255,0.72);
  --glass-dark:   rgba(10,7,2,0.86);
  --text:         #1A1000;
  --text-sub:     rgba(26,16,0,0.55);
  --gold:         #C9A84C;
  --gold-dark:    #8B6914;
  --gold-light:   #F0D080;
  ...
  --txt:          #1A1000;
  --txt-2:        #2D1E00;
  --txt-3:        rgba(26,16,0,0.52);
  --blue:         #C9A84C;
  --emerald:      #16A34A;
  --amber:        #D97706;
  --grad-bp:      linear-gradient(135deg,#C9A84C 0%,#F0D080 100%);
  --grad-surface: linear-gradient(145deg,rgba(255,255,255,0.9) 0%,rgba(255,255,255,0.75) 100%);
}
...
html,body {
  background: linear-gradient(145deg,#E8DEC8 0%,#DDD3B8 35%,#E4DAC4 70%,#EAE0CB 100%) !important;
...
[data-testid="stSidebar"] {
  ...
  box-shadow: 4px 0 48px rgba(0,0,0,0.45),
              inset -1px 0 0 rgba(201,168,76,0.08) !important;
...
.brand-icon {
  ...
  justify-content:center; color:#0C0800; font-weight:900;
  font-size:1.1rem; flex-shrink:0;
  box-shadow:0 4px 16px rgba(201,168,76,0.35);
}
.brand-name { font-weight:700; font-size:.95rem; color:#F0EAD8; font-family:'Inter',sans-serif; }
.brand-sub  { font-size:.55rem; letter-spacing:.15em; color:rgba(240,234,216,0.42);
              text-transform:uppercase; font-family:'Inter',sans-serif; }

/* Nav section label */
.sb-section {
  font-family:'Inter',sans-serif; font-size:.5rem; font-weight:700;
  color:rgba(201,168,76,0.38); letter-spacing:.22em; text-transform:uppercase;
...
.nav-item {
  ...
  letter-spacing:.1em; color:rgba(240,234,216,0.55);
  text-transform:uppercase; font-family:'Inter',sans-serif;
...
[data-testid="stSidebar"] .stButton > button:hover {
  ...
  border-left-color:rgba(201,168,76,0.45) !important;
  transform:none !important;
  box-shadow:inset 0 0 20px rgba(201,168,76,0.05) !important;
}
...
.nav-cta-wrap .stButton > button:hover {
  ...
  color:#F8E8A0 !important;
}
.nav-export-wrap .stButton > button {
  background:rgba(255,255,255,0.05) !important;
  color:rgba(240,234,216,0.7) !important;
...
.sb-status {
  ...
  font-weight:700; color:rgba(240,220,168,0.85); letter-spacing:.07em; text-transform:uppercase;
}
.sb-dot {
...
[data-testid="stSidebar"] [data-baseweb="select"] > div {
  ...
  color:#F0EAD8 !important; font-size:.8125rem !important;
}
...
.methodo-card {
  border-radius:20px;
  background:linear-gradient(135deg,#FFFDF5 0%,#FEF8E4 50%,#FFFDF5 100%);
  border:1px solid rgba(201,168,76,0.28);
  overflow:hidden; margin-bottom:2.25rem;
  box-shadow:0 8px 40px rgba(201,168,76,.12), 0 1px 0 rgba(201,168,76,.2) inset;
...
.methodo-eyebrow {
  ...
  color:rgba(139,105,20,.65); margin-bottom:.6rem;
}
.methodo-title {
  font-family:'Playfair Display',serif; font-size:1.35rem; font-weight:700;
  color:#4A2E00; line-height:1.3; margin-bottom:1.75rem;
}
.methodo-formula {
  background:rgba(201,168,76,.08);
  border:1px solid rgba(201,168,76,.28); border-radius:12px;
  padding:1.1rem 1.5rem; font-family:'Space Mono',monospace;
  font-size:.9rem; font-weight:700; color:#3D2B00;
...
.mf-gold { color:#8B6914; }
.mf-op   { color:rgba(0,0,0,.28); }
.methodo-pillars { display:grid; grid-template-columns:1fr 1fr 1fr; gap:1.5rem; margin-bottom:2rem; }
.mp {
  background:rgba(255,255,255,.75); border:1px solid rgba(201,168,76,.2);
...
.mp:hover { border-color:rgba(201,168,76,.45); box-shadow:0 4px 20px rgba(201,168,76,.15); }
.mp::before {
  content:''; position:absolute; top:0; left:0; right:0; height:2px;
  background:linear-gradient(90deg,#C9A84C,#F0D080); border-radius:14px 14px 0 0;
}
.mp-pct { font-family:'Space Mono',monospace; font-size:1.8rem; font-weight:700; color:#8B6914; line-height:1; margin-bottom:.5rem; }
.mp-name { font-family:'Playfair Display',serif; font-size:.85rem; font-weight:700; color:#3D2B00; margin-bottom:.75rem; }
.mp-bar-bg { height:4px; background:rgba(0,0,0,.07); border-radius:3px; overflow:hidden; margin-bottom:.85rem; }
...
.mp-sub { font-family:'Inter',sans-serif; font-size:.68rem; color:rgba(0,0,0,.52); line-height:1.6; }
.mp-sub strong { color:#7A5600; font-weight:600; }
.methodo-sources { border-top:1px solid rgba(201,168,76,.2); padding-top:1.25rem; display:flex; flex-wrap:wrap; gap:.5rem; align-items:center; }
.ms-label { font-family:'Inter',sans-serif; font-size:.5rem; font-weight:700; letter-spacing:.2em; text-transform:uppercase; color:rgba(139,105,20,.5); margin-right:.5rem; }
.ms-chip { font-family:'Space Mono',monospace; font-size:.6rem; color:#8B6914; background:rgba(201,168,76,.1); border:1px solid rgba(201,168,76,.22); border-radius:6px; padding:.2rem .6rem; letter-spacing:.05em; }
...
.pro-topbar {
  ...
  box-shadow:0 4px 32px rgba(0,0,0,0.3), inset 0 -1px 0 rgba(201,168,76,0.08);
...
.ptb-brand {
  ...
  color:#C9A84C; letter-spacing:.14em; text-transform:uppercase;
}
.ptb-sep { font-size:.75rem; color:rgba(255,255,255,0.18); }
.ptb-page {
  font-family:'Inter',sans-serif; font-size:.78rem; font-weight:600;
  color:rgba(240,234,216,0.88); letter-spacing:.02em;
}
.ptb-search-wrap {
...
.ptb-search-icon { color:rgba(201,168,76,0.55); flex-shrink:0; }
.ptb-search-input {
  flex:1; border:none; background:transparent; outline:none;
  font-family:'Inter',sans-serif; font-size:.76rem; color:rgba(240,234,216,0.9);
  min-width:0;
}
.ptb-search-input::placeholder { color:rgba(201,168,76,0.38); font-weight:400; }
.ptb-actions {
  display:flex; align-items:center; gap:.85rem; flex-shrink:0; margin-left:auto;
}
.ptb-divider { width:1px; height:22px; background:rgba(255,255,255,0.1); }
.ptb-bell {
...
.ptb-refresh {
  ...
  color:rgba(201,168,76,0.45); letter-spacing:.06em; white-space:nowrap;
}
.ptb-refresh-dot {
...
.ptb-avatar {
  ...
  display:flex; align-items:center; justify-content:center;
  font-family:'Space Mono',monospace; font-weight:700;
  font-size:.58rem; color:#0C0800; letter-spacing:.05em;
...
.glossary-panel {
  background:linear-gradient(145deg,#FFFDF5 0%,#FEF8E4 55%,#FFFDF5 100%);
  border:1px solid rgba(201,168,76,0.28); border-radius:18px;
  overflow:hidden; margin-bottom:1.5rem;
  animation:glossary-in .3s ease both;
  box-shadow:0 6px 32px rgba(201,168,76,.15), 0 1px 0 rgba(201,168,76,.2) inset;
}
.gp-shimmer {
  height:2px;
...
.gp-eyebrow {
  font-family:'Inter',sans-serif; font-size:.5rem; font-weight:700;
  letter-spacing:.25em; text-transform:uppercase; color:rgba(139,105,20,.6);
  margin-bottom:.35rem;
}
.gp-close {
  font-family:'Inter',sans-serif; font-size:.72rem; color:rgba(139,105,20,.65);
  text-decoration:none; border:1px solid rgba(201,168,76,.3); border-radius:8px;
  padding:.22rem .7rem; transition:all .2s;
}
.gp-close:hover { color:#4A2E00; border-color:rgba(201,168,76,.55); background:rgba(201,168,76,.1); }
.gp-grid {
  display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:1rem;
}
.gp-term {
  background:rgba(255,255,255,.8); border:1px solid rgba(201,168,76,.2);
  border-radius:12px; padding:1rem 1.1rem; transition:border-color .2s,box-shadow .2s;
}
.gp-term:hover { border-color:rgba(201,168,76,.45); box-shadow:0 4px 16px rgba(201,168,76,.12); }
.gp-term-name {
  font-family:'Space Mono',monospace; font-size:.72rem; font-weight:700;
  color:#8B6914; margin-bottom:.4rem; letter-spacing:.04em;
}
.gp-term-def {
  font-family:'Inter',sans-serif; font-size:.73rem; color:rgba(0,0,0,.62);
  line-height:1.6;
}
.gp-term-tag {
  display:inline-block; margin-top:.5rem;
  font-family:'Inter',sans-serif; font-size:.5rem; font-weight:700;
  letter-spacing:.12em; text-transform:uppercase; color:rgba(139,105,20,.55);
  background:rgba(201,168,76,.12); border-radius:5px; padding:.12rem .45rem;
}
</style>
...
with st.sidebar:
    ...
    <div style="margin-top:2rem;padding:1.25rem 1.25rem 1.5rem;
                ...
                background:rgba(201,168,76,0.06);border-radius:0 0 4px 4px">
      <div style="font-family:'Inter',sans-serif;font-size:.45rem;font-weight:700;
                  letter-spacing:.25em;text-transform:uppercase;
                  color:rgba(201,168,76,0.42);margin-bottom:.5rem">Aujourd'hui</div>
      <div style="display:flex;align-items:baseline;gap:.4rem">
        <div style="font-family:'Playfair Display',serif;font-size:2rem;font-weight:700;
                    color:#F0D080;line-height:1">{_day}</div>
        <div>
          <div style="font-family:'Inter',sans-serif;font-size:.65rem;font-weight:700;
                      color:#E8DCC8;letter-spacing:.08em">{_month}</div>
          <div style="font-family:'Space Mono',monospace;font-size:.55rem;
                      color:rgba(240,220,168,0.45);letter-spacing:.06em">{_year}</div>
        </div>
      </div>
      <div style="font-family:'Space Mono',monospace;font-size:.55rem;
                  color:rgba(201,168,76,0.38);margin-top:.35rem;letter-spacing:.1em">{_time} · Dakar, SN</div>
    </div>
    """, unsafe_allow_html=True)
...
_topbar_html = (
    ...
    '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#7B6030" stroke-width="2"'
